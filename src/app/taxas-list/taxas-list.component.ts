...
@Component({
  ...
  styleUrls: ['./taxas-list.component.scss'],
  providers: [],
  animations: [
    trigger('fadein', [
...
export class TaxasListComponent extends DataBasePrefabs implements OnInit {
  ...
  atualizar(event) {
    ...
    this.patch(event.value, '/taxa_extra').subscribe((data:any) => {
  ...
  cadastrar() {
    this.post(this.TEMPITEM.value, '/taxa_extra').subscribe(data => {
